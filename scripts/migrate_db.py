...
async def migrate_table(
    ...
    try:
        ...
        print(f"  ℹ Old table kept as {old_table} (drop manually if migration successful)")
        ...
        await conn.commit()
        print(f"  ✓ Rolled back to original table")
        return total, 0
...
async def migrate_database(db_path: Path, create_backup: bool = True) -> bool:
    ...
    async with aiosqlite.connect(db_path) as conn:
        ...
        print(f"  Success rate: {migrated_rows/total_rows*100:.1f}%" if total_rows > 0 else "N/A")
        print()
