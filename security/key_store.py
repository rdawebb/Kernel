...
class KeyStore:
    ...
    def _initialize_backend(self) -> None:
        ...
        for backend, check_func in backends:
            if check_func():
                ...
                log_event("keystore_initialized", {
    ...
    def _get_master_key(self) -> bytes:
        ...
        try:
            ...
            raise EncryptionError(
                "Failed to get or create master key",
                details={"error": str(e)}
            ) from e
    ...
    def _crypto_operation(self, value: str, operation: str) -> str:
        ...
        try:
            ...
            error_class = CorruptedSecretsError if operation == "decrypt" else EncryptionError
            message = (
    ...
    def _password_manager_operation(
        ...
        try:
            if operation == "store":
                ...
                if existing and existing.returncode == 0:
                    if manager == "op":
                        ...
                        subprocess.run(
                            ["bw", "edit", "item", item["id"], "--item", json.dumps(item)],
                            check=True,
                            capture_output=True
                        )
                else:
            ...
            if operation == "retrieve":
                return None
            raise KeyStoreError(
                f"Failed to {operation} credential in {manager}",
                details={"key": key}
            )
    ...
    def _file_credentials_operation(
        ...
        try:
            ...
            raise KeyStoreError(
                f"File operation failed for key '{key}'",
                details={"error": str(e)}
            ) from e
