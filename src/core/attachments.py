...
class AttachmentManager:
        ...
        for part in email_message.walk():
            ...
            try:
                ...
                raise AttachmentDownloadError(
                    f"Failed to extract attachment {filename}"
                ) from e
    ...
    def download_from_email_data(
        self, email_data: dict, attachment_index: Optional[int] = None
    ) -> List[Path]:
        ...
        try:
            ...
            if attachment_index is not None:
                if 0 <= attachment_index < len(attachments):
                    ...
                    raise InvalidPathError(
                        f"Invalid attachment index {attachment_index} (email has {len(attachments)} attachments)"
                    )
            ...
            raise AttachmentDownloadError(
                "Failed to download attachments from email data"
            ) from e

    def download_from_imap(
        self, imap_client, email_uid: str, attachment_index: Optional[int] = None
    ) -> List[Path]:
        ...
        try:
            ...
            if status != "OK" or not email_data or email_data[0] is None:
                raise AttachmentDownloadError(
                    f"Failed to fetch email UID {email_uid} from IMAP server"
                )
            ...
            raise AttachmentDownloadError(
                f"Failed to download attachments from email UID {email_uid}"
            ) from e

    def _save_attachments(
        self, attachments: List[Tuple[str, bytes]], email_uid: str
    ) -> List[Path]:
    ...
    def get_attachment_list_for_email(self, email_uid: str) -> List[str]:
        ...
        try:
            ...
            raise AttachmentNotFoundError(
                f"Failed to get attachment list for email UID {email_uid}"
            ) from e
    ...
    def open_attachment(self, filename: str) -> bool:
        ...
        try:
            ...
            raise AttachmentDownloadError(
                f"Failed to open attachment {file_path}"
            ) from e

        except FileNotFoundError as e:
            raise AttachmentDownloadError(
                f"No application found to open attachment {file_path}"
            ) from e

        except Exception as e:
            raise AttachmentDownloadError(
                f"Error opening attachment {file_path}"
            ) from e
