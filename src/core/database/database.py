...
from .schema import FIELD_MAPPING, SCHEMAS, QueryBuilder, TableSchema
...
class Database:
    ...
    async def _execute_many(
        self, query: str, param_list: List[Tuple], commit: bool = True
    ) -> None:
    ...
    def _extract_email_values(
        self, email: Dict[str, Any], schema: TableSchema
    ) -> List[Any]:
    ...
    async def _process_in_batches(
        ...
        for i in range(0, total, batch_size):
            ...
            try:
                ...
                if total > batch_size and (i + batch_size) % (batch_size * 5) == 0:
                    logger.info(
                        f"{operation_name}: Processed {processed_count}/{total} items"
                    )
        ...
        logger.info(
            f"{operation_name} complete: {processed_count}/{total} items processed"
        )
    ...
    async def save_email(self, table: str, email: Dict[str, Any]) -> None:
        ...
        try:
            await self._execute_query(
                query, tuple(values), fetch_all=False, commit=True
            )
    ...
    @async_log_call
    async def get_email(
        self, table: str, uid: str, include_body: bool = True
    ) -> Optional[Dict]:
        ...
        try:
            return await self._execute_query(query, (str(uid),), fetch_one=True)
    ...
    async def get_emails(
        ...
        try:
            ...
            raise DatabaseError(
                f"Failed to retrieve emails from {table}", details={"error": str(e)}
            ) from e

    @async_log_call
    async def get_emails_by_uids(
        self,
        table: str,
        uids: List[str],
    ...
    async def delete_emails_batch(
        self, table: str, uids: List[str], batch_size: int = DEFAULT_BATCH_SIZE
    ) -> int:
        ...
        async def delete_batch(batch_uids: List[str]) -> int:
            ...
            await self._execute_query(
                query, tuple(batch_uids), fetch_all=False, commit=True
            )
            return len(batch_uids)
    ...
    async def move_email(
        self, source_table: str, dest_table: str, uid: str, **extra_fields
    ) -> None:
    ...
    async def move_emails_batch(
        ...
        **extra_fields,
        ...
        async def move_batch(batch_uids: List[str]) -> int:
            ...
            all_values = [
                tuple(self._extract_email_values(email, dest_schema))
                for email in emails
            ]
            insert_placeholders = QueryBuilder.build_placeholders(
                len(dest_schema.all_columns)
            )
            ...
            await self._execute_query(
                delete_query, tuple(batch_uids), fetch_all=False, commit=True
            )
        ...
        return await self._process_in_batches(
            ...
            batch_size,
            move_batch,
            f"Moving emails from {source_table} to {dest_table}",
        )
    ...
    async def update_field(self, table: str, uid: str, field: str, value: Any) -> None:
        ...
        try:
            await self._execute_query(
                query, (value, str(uid)), fetch_all=False, commit=True
            )
    ...
    async def update_fields_batch(
        self, table: str, updates: List[Dict[str, Any]], batch_size: int = 100
    ) -> int:
    ...
    async def search_all_tables(self, keyword: str, limit: int = 50) -> List[Dict]:
        ...
        query = (
            " UNION ALL ".join(union_parts) + " ORDER BY date DESC, time DESC LIMIT ?"
        )
        params.append(limit)
    ...
    async def search_with_attachments(
        self, table: str = "inbox", limit: int = 50
    ) -> List[Dict]:
    ...
    async def get_highest_uid(self) -> Optional[int]:
        ...
        try:
            ...
            return (
                int(result["max_uid"])
                if result and result["max_uid"] is not None
                else None
            )
    ...
    async def get_email_count(self, table: str) -> int:
        ...
        try:
            ...
            raise DatabaseError(
                f"Failed to get email count from {table}", details={"error": str(e)}
            ) from e
    ...
    async def backup(self, backup_path: Optional[Path] = None) -> Path:
        ...
        try:
            ...
            raise DatabaseError(
                "Failed to backup database", details={"error": str(e)}
            ) from e
    ...
    async def export_to_csv(
        self, export_dir: Path, tables: Optional[List[str]] = None
    ) -> List[Path]:
        ...
        try:
            ...
            for table in tables:
                ...
                emails = await self.get_emails(
                    table, limit=self.MAX_EXPORT_LIMIT, include_body=True
                )
            ...
            raise DatabaseError(
                "Failed to export database to CSV", details={"error": str(e)}
            ) from e
    ...
    async def delete_database(self) -> None:
        ...
        try:
            ...
            raise DatabaseError(
                "Failed to delete database", details={"error": str(e)}
            ) from e
