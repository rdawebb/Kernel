...
def setup_viewing_commands(subparsers) -> None:
    ...
    drafts_parser = subparsers.add_parser(
        "drafts", help="View draft emails", description="Display draft emails"
    )
    add_limit_argument(drafts_parser, default=10)
    add_filter_arguments(drafts_parser)

    trash_parser = subparsers.add_parser(
        "trash", help="View deleted emails", description="Display emails in trash"
    )
...
def setup_email_commands(subparsers) -> None:
    ...
    email_subparsers = email_parser.add_subparsers(
        dest="email_command", required=True, help="Email operation to perform"
    )
...
def setup_search_command(subparsers) -> None:
    ...
    search_parser.add_argument(
        "--all", action="store_true", help="Search in all folders"
    )
...
def setup_attachment_commands(subparsers) -> None:
    ...
    attachments_subparsers = attachments_parser.add_subparsers(
        dest="attachment_command", help="Attachment operation to perform"
    )
    ...
    open_parser = attachments_subparsers.add_parser(
        "open", help="Open a downloaded attachment"
    )
...
def setup_compose_commands(subparsers) -> None:
    ...
    compose_parser.add_argument(
        "--to", help="Recipient email addresses (comma-separated)"
    )
    compose_parser.add_argument("--subject", help="Email subject")
    compose_parser.add_argument("--body", help="Email body content")
    compose_parser.add_argument(
        "--cc", action="append", help="CC recipient email addresses (comma-separated)"
    )
    compose_parser.add_argument(
        "--bcc", action="append", help="BCC recipient email addresses (comma-separated)"
    )
...
def setup_maintenance_commands(subparsers) -> None:
    ...
    refresh_parser.add_argument(
        "--all", action="store_true", help="Fetch all emails, not just new ones"
    )
    ...
    db_subparsers = db_parser.add_subparsers(
        dest="db_command", required=True, help="Database operation to perform"
    )
    ...
    delete_parser = db_subparsers.add_parser("delete", help="Delete the local database")
    delete_parser.add_argument(
        "--confirm",
        action="store_true",
        required=True,
...
def setup_config_commands(subparsers) -> None:
    ...
    config_subparsers = config_parser.add_subparsers(
        dest="config_command", required=True, help="Configuration operation to perform"
    )

    list_parser = config_subparsers.add_parser("list", help="List current settings")
    ...
    reset_parser = config_subparsers.add_parser(
        "reset", help="Reset settings to default"
    )
    reset_parser.add_argument(
        "--key", nargs="?", help="Specific config key to reset (omit to reset all)"
    )
