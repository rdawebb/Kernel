...
from src.features.view import EmailFilters, view_email, view_folder
...
class CommandRouter:
    ...
    async def _handle_folder(self, folder: str, args: Dict[str, Any]) -> bool:
        ...
        return await view_folder(
            folder=folder, limit=limit, filters=filters, console=self.console
        )
    ...
    async def _handle_email_delete(self, args: Dict[str, Any]) -> bool:
        ...
        return await delete_email(
            email_id=email_id, folder=folder, permanent=permanent, console=self.console
        )
