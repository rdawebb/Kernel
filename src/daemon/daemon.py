...
class EmailConnectionPool:
    ...
    def get_client(self) -> Any:
        ...
        if self.pool.is_expired(current_time):
            ...
            log_event(
                f"{self._client_type.lower()}_connected", {"timeout": self.pool.timeout}
            )
...
class ConnectionManager:
    ...
    async def keepalive_loop(self) -> None:
        ...
        while True:
            ...
            # Keepalive for IMAP
            await safe_execute(
                self.imap_pool.keepalive, default=None, context="imap_keepalive"
            )
...
class CacheManager:
    ...
    async def invalidate_by_pattern(self, pattern: str) -> int:
        ...
        async with self._lock:
            ...
            if to_remove:
                logger.info(
                    f"Cache invalidated by pattern '{pattern}': {len(to_remove)} entries"
                )
                log_event(
                    "cache_invalidated_pattern",
                    {"pattern": pattern, "entries_removed": len(to_remove)},
                )
...
class EmailDaemon:
        ...
        try:
            ...
            log_event(
                "daemon_initialized",
                {
                    ...
                    "max_concurrent_clients": self.RESOURCE_LIMITS[
                        "max_concurrent_clients"
                    ],
                },
            )
    ...
    async def _invalidate_cache_selective(
        ...
        if strategy == "all":
            ...
            logger.warning(
                f"Unknown invalidation strategy for '{command}', clearing all cache"
            )
            await self.cache.invalidate_all()
        ...
        try:
            ...
            while await self.get_active_clients() > 0:
                ...
                if time.time() - shutdown_start > 30:
                    ...
                    self.logger.warning(
                        f"Timeout waiting for {active} clients, forcing shutdown"
                    )
                    break
