...
class TestDatabaseManagement:
    ...
    def test_initialize_db_creates_tables(self, temp_db, mock_config):
        ...
        with patch('src.core.db_manager.DatabaseManager.get_db_path', return_value=temp_db):
...
class TestEmailFlagging:
    ...
    def test_mark_email_unflagged(self, temp_db, mock_config):
        ...
        with patch('src.core.db_manager.DatabaseManager.get_db_path', return_value=temp_db):
            ...
            email = DatabaseTestHelper.create_mock_email(uid='test_unflag', flagged=True)
            storage_api.save_email_to_table('inbox', email)
...
class TestEmailMovement:
    ...
    def test_move_email_to_deleted(self, temp_db, mock_config):
        ...
        with patch('src.core.db_manager.DatabaseManager.get_db_path', return_value=temp_db):
            ...
            # Move to deleted
            storage_api.move_email_between_tables('inbox', 'deleted_emails', email['uid'])
            
            # Verify moved
            assert storage_api.get_email_from_table('inbox', email['uid']) is None
            assert storage_api.get_email_from_table('deleted_emails', email['uid']) is not None
